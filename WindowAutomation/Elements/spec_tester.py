...
# Phiên bản 8.0: Thêm màu sắc cho log [GIỮ LẠI] và [LOẠI BỎ] để dễ đọc hơn.
...
class SpecTesterApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Selector Debugger v8.0")
    ...
    'pwa_class_name': ('icontains', 'CabinetWClass')
        ...
        # --- NÂNG CẤP: Thêm tag màu cho GIỮ LẠI / LOẠI BỎ ---
        self.log_area.tag_config('KEEP', foreground='#76D7C4', font=("Consolas", 10, "bold")) # Màu xanh ngọc
        self.log_area.tag_config('DISCARD', foreground='#E59866', font=("Consolas", 10, "bold")) # Màu cam
    ...
    def log_message(self, level, message):
        ...
        # Cho phép message là một list các tuple (text, tag)
...
class SelectorDebugger:
    ...
    def _apply_filters_debug(self, elements, spec):
        ...
        for key, criteria in spec.items():
            ...
            if logged_count >= log_limit and initial_count > log_limit: self.log('DEBUG', f"  ... và {initial_count - log_limit} ứng viên khác được xử lý.")
            self.log('INFO', f"  -> Kết quả: Giữ lại {len(kept_elements)}/{initial_count} ứng viên.")
            if not kept_elements: self.log('ERROR', f"Bộ lọc này đã loại bỏ tất cả ứng viên."); return []
    ...
    def _get_process_info(self, pid):
        ...
        if pid > 0:
            try:
                ...
                info = {
                    'name': p.name(), 'path': p.exe(), 'cmdline': ' '.join(p.cmdline()),
                    'create_time': datetime.fromtimestamp(p.create_time()), 'username': p.username()
                }
                self._proc_info_cache[pid] = info; return info
